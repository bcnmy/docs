...
:::tip
You can also use this module as the **default module** just like the ECDSA Validation Module, as it has all its capabilities for sending ops on the particular chain.
:::
...
3. Cross chain state sync actions e.g transfer ownership of a smart account, change ENS, similar actions on protocol that is deployed on multiple chains
4. Onboard a user on x chains by deploying their smart account and some action like register in a raffle 
...
:::info
If the Smart Account has not been deployed yet then it makes sense to pass multiChainModule as defaultValidationModule. This ensures its default activation and has all the capabilities of the ECDSA Module (popular go-to module). 
:::
...
"Multichain module allows us to use session keys and setup sessions on multiple chains with single wallet signature." - [Rage Trade](https://www.rage.trade/)