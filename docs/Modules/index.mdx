...
- [**Session key Manager Validation Module**](./sessions/sessionvalidationmodule.mdx): This module enables the use of sessions to execute transactions. It verifies that session is on-chain and the operation has been signed by that session key and whether a given user operation adheres to the permissions defined within the session key using sub-modules contracts called session validation modules. This can only be used as an active validation module.
