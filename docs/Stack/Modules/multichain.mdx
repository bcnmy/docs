...
This module acts as an extension of the [ECDSA Module](/stack/modules/ecdsa). The added benefit of this module is you now have the ability to sign once and authorize any number of userOps across multiple blockchain networks.
...
const provider = new new ethers.JsonRpcProvider("[RPC_Endpoint]")();
...
Once initialized it can be passed to the smart account [create method](/stack/account/methods#create) to create instances of the smart account.
Check [Paymaster integration](/stack/paymaster/integration) and [Bundler integration](/stack/bundler/integration) sections to create instances of them.
...
const returnedOps = await multiChainModule.signUserOps([
  { userOp: partialUserOp1, chainId: ChainId.BASE_GOERLI_TESTNET },
