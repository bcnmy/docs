...
import { ISessionStorage, SessionLeafNode, SessionSearchParam, SessionStatus } from "@biconomy/modules/dist/src/interfaces/ISessionStorage";
import { mainnet } from "viem/chains";
import { generatePrivateKey, privateKeyToAccount } from "viem/accounts";
import { SignerData } from "@biconomy/modules";
...
You can now assing a new session storage client to the module.
...
```typescript
  ...
  const sessionModule = await SessionKeyManagerModule.create({
    ...
    sessionStorageClient: sessionFileStorage, // Now we specify the new session storage client
  });
