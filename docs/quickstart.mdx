...
Let's create a [Typescript](https://www.typescriptlang.org) project using npm to get started.

```bash
mkdir biconomy-sa && cd biconomy-sa && npm init -y
```

**Install biconomy's account package using npm:**
...
And install some helper dev dependencies...
